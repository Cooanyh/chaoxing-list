...
### v2.4.9
* ✅ **修复任务引擎考试完成状态**：任务引擎考试跳转到已提交答卷查看页时识别为“已完成”，不再因计划明细状态滞后显示“进行中”。

### v2.4.8
* 🪟 **修复详情页悬浮卡重复显示**：课程进度详情使用页面级浮层时隐藏卡片内原始悬浮层，避免同一课程同时出现两张详情卡。
